...
const FULL_TIMELINE_START = new Date(YEAR, 3, 1);
const FULL_TIMELINE_END = new Date(YEAR, 8, 30, 23, 59, 59);
...
function createTimelineContext(now) {
  const dynamic = currentFilter === 'upcoming';
  const requestedStart = dynamic ? new Date(YEAR, now.getMonth() - 1, 1) : FULL_TIMELINE_START;
  const start = requestedStart > FULL_TIMELINE_START ? requestedStart : FULL_TIMELINE_START;
  const end = FULL_TIMELINE_END;
  ...
  return {
    ...
    dynamic,
    pct(date) {
      const bounded = Math.max(start.getTime(), Math.min(end.getTime(), date.getTime()));
      return ((bounded - start.getTime()) / duration) * 100;
    },
  };
}
  ...
  return MODULES.map((module) => ({
    ...
    items: module.assessments.map((assessment) => {
      ...
      return {
        ...
        openNow,
        status,
        datesCustomised:Boolean(overrideDates),
        completionOverride,
      };
    }),
  }));
...
function render() {
  ...
  $('donePct').textContent = `${percentage}% complete`;
  ...
  if (next) {
    ...
    $('nextTitle').textContent = 'All caught up';
    $('nextDue').textContent = 'No upcoming assessments';
    $('nextCountdown').textContent = '🎉';
  }

  renderModules(modules, now, next, createTimelineContext(now));
}

function createTimelineHeader(context) {
  const header = document.createElement('div');
  header.className = 'timeline-header';
  const label = document.createElement('div');
  label.className = 'timeline-label';
  label.textContent = context.dynamic ? `Timeline · from ${formatDate(context.start, { month:'short' })}` : 'Timeline';
  const scale = document.createElement('div');
  scale.className = 'timeline-scale';
  for (const date of context.months) {
    const left = context.pct(date);
    const tick = document.createElement('div');
    tick.className = 'month-tick';
    tick.style.left = `${left}%`;
    const month = document.createElement('div');
    month.className = 'month-label';
    month.style.left = `${left}%`;
    month.textContent = formatDate(date, { month:'short' });
...
function createTimelineGrid(context) {
  const grid = document.createElement('div');
  grid.className = 'timeline-grid';
  for (const date of context.months) {
    const tick = document.createElement('div');
    tick.className = 'month-tick';
    tick.style.left = `${context.pct(date)}%`;
    grid.append(tick);
  }
  return grid;
}

function addTodayLine(timeline, now, showLabel, context) {
  if (now < context.start || now > context.end) return;
  const line = document.createElement('div');
  line.className = `today-line${showLabel ? '' : ' no-label'}`;
  line.style.left = `${context.pct(now)}%`;
  timeline.append(line);
}

function renderModules(modules, now, next, context) {
  const container = $('tracker');
  container.replaceChildren(createTimelineHeader(context));
  let lastLevel = null;
  let visible = 0;
  ...
  for (const module of modules) {
    ...
    if (module.level !== lastLevel) {
      ...
      level.className = 'level-title';
      level.textContent = `Level ${module.level}`;
      container.append(level);
      lastLevel = module.level;
    }
    ...
    link.href = `https://mymodules.dtls.unisa.ac.za/course/view.php?id=${module.moodleId}`;
    link.target = '_blank';
    link.rel = 'noopener';
    link.style.color = module.color;
    link.textContent = `${module.code} ↗`;
    code.append(link);
    const title = document.createElement('div');
    title.className = 'module-title';
    title.textContent = module.title;
    const progress = document.createElement('div');
    progress.className = 'module-progress';
    progress.textContent = `${module.items.filter((item) => item.isDone).length}/${module.items.length} done`;
    info.append(code, title, progress);

    const timeline = document.createElement('div');
    timeline.className = 'timeline';
    timeline.append(createTimelineGrid(context));
    for (const item of items) {
      const lane = document.createElement('div');
      lane.className = 'timeline-lane';
      lane.append(createAssessmentBar(item, next, context));
      timeline.append(lane);
    }
    addTodayLine(timeline, now, !todayLabelShown, context);
  ...
  if (!visible) {
    ...
    empty.textContent = 'No assessments match this filter.';
    container.append(empty);
  }
}

function createAssessmentBar(item, next, context) {
  ...
  if (!item.dueDate) {
    ...
    const left = context.pct(item.openDate);
    const width = Math.max(2, context.pct(item.dueDate) - left);
    button.style.left = `${left}%`;
    button.style.width = `${width}%`;
    button.textContent = `A${item.n} · ${formatDate(item.openDate)} → ${formatDate(item.dueDate)}`;
  } else {
    button.classList.add('point');
    button.style.left = `calc(${context.pct(item.dueDate)}% - 7px)`;
  }
...
function openEditor(item, trigger) {
  ...
  $('editOpen').value = toISO(item.openDate);
  $('editDue').value = toISO(item.dueDate);
...
async function toggleCompletion() {
  ...
  const original = Boolean(MODULES.find((module) => module.code === pendingItem.moduleCode)?.assessments.find((assessment) => assessment.n === pendingItem.n)?.done);
  if (target === original) delete state.completion[pendingItem.key];
  else state.completion[pendingItem.key] = target ? 'done' : 'undone';
  if (await saveState(snapshot)) {
    closeEditor();
    render();
  }
}
async function saveDates() {
  ...
  const snapshot = structuredClone(state);
  const open = $('editOpen').value || null;
  const due = $('editDue').value;
...
function updateLockUI() {
  $('lockBtn').textContent = authed ? '🔓 Lock editing' : '🔒 Unlock to edit';
  $('lockBtn').dataset.authed = authed ? '1' : '0';
...
async function submitAuth() {
  const { response, body } = await api('/api/auth', {
    method:'POST',
    headers:{'Content-Type':'application/json'},
  ...
  $('authError').textContent = response.status === 429 ? 'Too many attempts. Please wait 15 minutes.' : body?.error === 'server_not_configured' ? 'Authentication is not configured.' : 'Incorrect password.';
}
...
function bindEvents() {
  ...
  $('modalToggle').addEventListener('click', toggleCompletion);
  $('modalSave').addEventListener('click', saveDates);
  $('modalClearDates').addEventListener('click', () => {
    if (!pendingItem) return;
    $('editOpen').value = toISO(pendingItem.originalOpen);
    $('editDue').value = toISO(pendingItem.originalDue);
    validateDates();
  });
  $('editOpen').addEventListener('input', validateDates);
  $('editDue').addEventListener('input', validateDates);
  ...
  $('exportButton').addEventListener('click', () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `unisa-tracker-state-${toISO(new Date())}.json`;
    link.click();
    URL.revokeObjectURL(url);
  });
  $('resetButton').addEventListener('click', async () => {
    if (!authed || !confirm('Clear all status and date overrides?')) return;
    const snapshot = structuredClone(state);
    state = stateEmpty();
    if (await saveState(snapshot)) render();
  });
  for (const modal of [$('editorModal'), $('authModal')]) {
    modal.addEventListener('click', (event) => {
      if (event.target === modal) modal === $('editorModal') ? closeEditor() : closeAuth();
    });
  }
